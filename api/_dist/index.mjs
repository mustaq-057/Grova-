...
var require_ece = __commonJS({
  "../../node_modules/.pnpm/http_ece@1.2.0/node_modules/http_ece/ece.js"(exports, module) {
    ...
    function decrypt2(buffer, params, keyLookupCallback) {
      ...
      for (var i = 0; start2 < buffer.length; ++i) {
        var end = start2 + chunkSize;
        if (header.version !== "aes128gcm" && end === buffer.length) {
          throw new Error("Truncated payload");
        }
        end = Math.min(end, buffer.length);
        if (end - start2 <= TAG_LENGTH) {
          throw new Error("Invalid block: too small at " + i);
        }
        var block = decryptRecord(
          key,
          i,
          buffer.slice(start2, end),
          header,
          end >= buffer.length
        );
    ...
    function encrypt2(buffer, params, keyLookupCallback) {
      ...
      while (!last) {
        ...
        pad -= recordPad;
        var end = start2 + header.rs - overhead - recordPad;
        if (header.version !== "aes128gcm") {
...
function validateEnv() {
  ...
  const encryptionPassword2 = (process.env.ENCRYPTION_PASSWORD || "").trim();
...
function generateTurnCredentials(username, secret, ttl = 86400) {
  ...
  const hmac = crypto2.createHmac("sha1", secret);
...
function startScheduleWorker() {
  ...
  setInterval(runScheduleTick, 3e4);
...
router25.delete("/hidden-messages/:userId/restore", rateLimiters.messages, authenticate, async (req, res) => {
  const authId = req.user.id;
  const userId = String(req.params.userId);
  if (userId !== authId) {
    res.status(403).json({ error: "Forbidden" });
  ...
  try {
    // Remove all hidden message IDs for this user
    await db_default.execute(
      "DELETE FROM hidden_messages WHERE user_id = $1",
      [userId]
    );
    // Clear the clearedAt timestamp so all old messages are visible again
    await setChatClearedForUser(userId, null);
    res.json({ success: true });
  } catch (err) {
...
router31.use(sanitizeInput);
router31.use(csrfProtection);
router31.use(validateRouteParams({}));
router31.use(health_default);
router31.use(events_default);
router31.use(calendar_events_default);
router31.use(checkins_default);
router31.use(tasks_default);
router31.use(milestones_default);
router31.use(secret_notes_default);
router31.use(presence_default);
router31.use(profile_default);
router31.use(auth_default);
router31.use(messages_default);
router31.use(duas_default);
router31.use(call_default);
router31.use(images_default);
router31.use(notifications_default);
router31.use(twoFactor_default);
router31.use(reactions_default);
router31.use(typing_default);
router31.use(readReceipts_default);
router31.use(forward_default);
router31.use(edit_default);
router31.use(pin_default);
router31.use(schedule_default);
router31.use(media_default);
router31.use(hidden_messages_default);
router31.use(export_default);
router31.use(couple_sync_default);
router31.use(library_default);
router31.use("/doodle", doodle_default);
router31.use(stories_default);
router31.use(avatar_notes_default);
