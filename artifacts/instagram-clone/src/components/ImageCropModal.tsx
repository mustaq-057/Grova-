...
export const ImageCropModal = memo(function ImageCropModal({
  ...
  const processAiRetouch = async () => {
    ...
    setProcessing(true);
      ...
      // Bypasses APIs completely for instant, free results
      setTimeout(() => {
        try {
          const img = new Image();
      ...
      <div className="flex-1 relative w-full overflow-hidden min-h-0 bg-[#0a0a0a]" ref={containerRef}>
        ...
        {processing && (
          <div className="absolute inset-0 z-[1000] bg-black/70 backdrop-blur-md flex flex-col items-center justify-center animate-in fade-in rounded-[40px]">
             ...
             <span className="mt-4 text-white font-bold tracking-widest uppercase text-sm animate-pulse">Removing Object...</span>
          </div>
        )}
